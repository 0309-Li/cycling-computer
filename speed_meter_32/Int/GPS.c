...
void updata_pos_async_cb(void *arg)
{
    ...
    lv_subject_set_int(&speed_meter_sub->gps, sa_num);
    GPS_Transform(latitude, longitude, &latitude, &longitude);
    // ui_update_pos(ui, latitude, longitude);
    lv_subject_set_float(&speed_meter_sub->latitude, latitude);
    lv_subject_set_float(&speed_meter_sub->longitude, longitude);
}
...
void updata_distance_async_cb(void *arg)
{
    ...
    lv_subject_set_float(&speed_meter_sub->mile_sub, distance);
}

void updata_ride_time_async_cb(void *arg)
{
    // ui_update_ride_time(ui, ride_time_s);
    lv_subject_set_int(&speed_meter_sub->min_sub, ride_time_s / 60);
    lv_subject_set_int(&speed_meter_sub->sec_sub, (uint32_t)ride_time_s % 60);
}
...
void updata_status_async_cb(void *arg)
{
    ...
    lv_subject_set_pointer(&speed_meter_sub->sport_status, (void *)sport_status[status]);
}
